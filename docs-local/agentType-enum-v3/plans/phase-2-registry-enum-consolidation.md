...
**Branch**: `refactor/agent-registry-consolidation` off `main` (after Phase 1 merged)
**PR Target**: `main`
**Depends On**: Phase 1 MVF must be merged first

---

## Prerequisites

Phase 1 must be merged. Verify:
```bash
...
```python
from claude_mpm.services.agents.agent_name_registry import CORE_AGENT_IDS
```
...
### File: `src/claude_mpm/services/agents/agent_name_registry.py`
...
python -c "
from claude_mpm.services.agents.agent_name_registry import get_agent_name_map
...
```bash
git add src/claude_mpm/services/agents/agent_name_registry.py
...
import pytest
from claude_mpm.services.agents.agent_name_registry import (
    AGENT_NAME_MAP,
    get_agent_name_map,
    invalidate_cache,
    CORE_AGENT_IDS,
